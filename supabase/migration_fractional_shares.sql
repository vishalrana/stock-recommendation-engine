-- Migration: Enable Fractional Shares Support
-- Description: Alters max_shares column from INTEGER to NUMERIC(12, 4) in signals and signals_history tables,
-- and updates execute_position_exit RPC function to handle fractional share lot splitting.

ALTER TABLE signals ALTER COLUMN max_shares TYPE NUMERIC(12, 4);
ALTER TABLE signals_history ALTER COLUMN max_shares TYPE NUMERIC(12, 4);
...
BEGIN
  ...
  shares_sold := CASE
    ...
    ELSE round((COALESCE(s.max_shares, 0) * split_fraction)::numeric, 4)
  END;
  dollars_sold := CASE
    WHEN split_fraction = 1 THEN COALESCE(s.allocated_dollars, 0)
    ELSE round((COALESCE(s.allocated_dollars, 0) * split_fraction)::numeric, 2)
  END;
