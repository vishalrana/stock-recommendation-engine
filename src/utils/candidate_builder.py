...
def build_candidate_from_row(row: pd.Series) -> dict:
    ...
    return {
        ...
        "target_1_atr": float(row.get("target_1_atr", 0.0)) if pd.notna(row.get("target_1_atr")) else None,
        "target_2_atr": float(row.get("target_2_atr", 0.0)) if pd.notna(row.get("target_2_atr")) else None,
        "target_3_atr": float(row.get("target_3_atr", 0.0)) if pd.notna(row.get("target_3_atr")) else None,
        "target_1_pct": float(row.get("target_1_pct", 0.0)) if pd.notna(row.get("target_1_pct")) else None,
        "target_2_pct": float(row.get("target_2_pct", 0.0)) if pd.notna(row.get("target_2_pct")) else None,
        "target_3_pct": float(row.get("target_3_pct", 0.0)) if pd.notna(row.get("target_3_pct")) else None,
        "reach_prob_t1": float(row.get("reach_prob_t1", 0.0)) if pd.notna(row.get("reach_prob_t1")) else None,
        "reach_prob_t2": float(row.get("reach_prob_t2", 0.0)) if pd.notna(row.get("reach_prob_t2")) else None,
        "reach_prob_t3": float(row.get("reach_prob_t3", 0.0)) if pd.notna(row.get("reach_prob_t3")) else None,
