...
WITH unified_signals AS (
  ...
  SELECT
    ...
    COALESCE(h.context_analyst, 0.0) AS context_analyst,
    COALESCE(h.context_earnings, 0.0) AS context_earnings,
    COALESCE(h.context_news, 0.0) AS context_news,
    COALESCE(h.context_fundamental, 0.0) AS context_fundamental,
    h.allocated_dollars,
    h.max_shares
